...
@interface GKGLMemberBenefitsVC ()<SPPageMenuDelegate, UIScrollViewDelegate>
...
@implementation GKGLMemberBenefitsVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.view.backgroundColor = kColorE;
    [self.navView setNavViewTitle:@"会员权益" backBtnShow:YES];
...
- (void)initSubViews
{
    ...
    for (int i = 0; i < _dataArr.count; i++) {
        GKGLMemberBenefitsSubVC * vc = [[GKGLMemberBenefitsSubVC alloc] init];
        vc.userid = _userid;
        [self addChildViewController:vc];
        vc.tag = i;
        vc.GKGLMemberBenefitsSubVCBlock = ^(NSString * _Nonnull count) {
            if (vc.tag == 0) {
                [weakSelf.pageMenu setTitle:[NSString stringWithFormat:@"可使用（%@）",count] forItemAtIndex:0];
            }
            if (vc.tag == 1) {
                [weakSelf.pageMenu setTitle:[NSString stringWithFormat:@"已过期（%@）",count] forItemAtIndex:1];
            }
        };
        [self.myChildViewControllers addObject:vc];
    }
    if (self.pageMenu.selectedItemIndex < self.myChildViewControllers.count) {
        GKGLMemberBenefitsSubVC *baseVc = self.myChildViewControllers[self.pageMenu.selectedItemIndex];
...
//    }
    ...
    GKGLMemberBenefitsSubVC *targetViewController = self.myChildViewControllers[toIndex];
    // 如果已经加载过，就不再加载
