...
@implementation LanternXHPlanVC
...
//        _detailWebView.scrollView.scrollEnabled = NO;
        ...
        [_detailWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@intelligence/consume.html",SERVER_H5]]]];
    }
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
    ...
    NSString * jsStr = [NSString stringWithFormat:@"LanternCallIos('%@','%@','%@','%@','%@')",token,joinCode,@"2",infomodel.data.account,date];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    if ([message.name isEqualToString:@"LanternGoBack"]) {
        ...
        if (_comeFrom == DetailComeFromeMakePlan) {
            if (_LanternXHPlanVCBlock) {
                _LanternXHPlanVCBlock(editID,@"1");
            }
            [self.navigationController popViewControllerAnimated:NO];
        }else if (_comeFrom == DetailComeFromeHistory){
