...
+ (NSString *)dayFromWeekdayEnglish:(NSDate *)date {
    switch([date weekday]) {
        case 1:
            return @"SUN";
            break;
...
/**
 ...
 @return array <array>
 */
- (NSArray *)getAllWeekDate:(NSDate *)selectDate weekOfYear:(NSInteger *)paramWeekOfYear {
    
    NSDate *currentDate = NSDate.date;
    
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateComponents *comp = [calendar components: NSCalendarUnitWeekday | NSCalendarUnitWeekOfYear fromDate:self];
    // 获取今天是周几 (周一是2，周日是1)
    NSInteger weekDay = [comp weekday];
    //这周是今年的第几周(注意：第一周编号为1不为0)
    NSInteger weekOfYear = [comp weekOfYear];
    
    *paramWeekOfYear = weekOfYear;
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyyMMdd"];
    NSDate *date = [formatter dateFromString:[formatter stringFromDate:selectDate]];
    //这周的开始和结束时间 （注意：+1 -1 避开临界值）
    NSDate *startdate = [date dateByAddingTimeInterval:24*3600*(2 - weekDay)];
    NSDate *enddate = [date dateByAddingTimeInterval:24*3600*(8 - weekDay)];
    ...
    for (int i = 1; i <= 52; i++) {
        ...
        NSMutableDictionary *weekDic = NSMutableDictionary.new;
        [dateArray addObject:weekDic];
        weekDic[@"startDate"] = startdate1;
        weekDic[@"endDate"] = enddate1;
//        [dateArray addObject:@[startdate1,enddate1]];
        if (currentDate.timeIntervalSince1970 >= startdate1.timeIntervalSince1970) {
            weekDic[@"enable"] = @(YES);
        } else {
            weekDic[@"enable"] = @(NO);
            
            // 超过当前时间。停止
//            [dateArray removeObject:dateArray.lastObject];
//            break;
        }
    }
    return dateArray;
