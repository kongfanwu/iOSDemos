...
@interface GKGLCustomerCFVC ()<SPPageMenuDelegate, UIScrollViewDelegate>
...
@implementation GKGLCustomerCFVC
...
- (void)initSubViews
{
    ...
    for (int i = 0; i < _dataArr.count; i++) {
        GKGLCustomerCFSubVC * vc = [[GKGLCustomerCFSubVC alloc] init];
        vc.userid = _userid;
    ...
    if (self.pageMenu.selectedItemIndex < self.myChildViewControllers.count) {
        GKGLCustomerCFSubVC *baseVc = self.myChildViewControllers[self.pageMenu.selectedItemIndex];
...
- (void)pageMenu:(SPPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex {
    ...
    GKGLCustomerCFSubVC *targetViewController = self.myChildViewControllers[toIndex];
    // 如果已经加载过，就不再加载
