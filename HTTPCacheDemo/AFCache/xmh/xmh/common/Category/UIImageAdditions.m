...
+ (UIImage *) mergeImages:(NSArray *)images logo:(UIImage *)logo {
	...
	for ( UIImage *image in images) {
		CGFloat iWidth = CGImageGetWidth([image CGImage]);
		CGFloat iHeight = CGImageGetHeight([image CGImage]);
	...
	for ( UIImage *img in images) {
		...
		CGFloat iWidth = CGImageGetWidth([img CGImage]);
		CGFloat iHeight = CGImageGetHeight([img CGImage]);
...
//
//	CGSize imageSize = self.size;
////    CGContextRef shadowContext = CGBitmapContextCreate(NULL, self.size.width + w, self.size.height + h, CGImageGetBitsPerComponent(self.CGImage), 0,colourSpace,kCGImageAlphaPremultipliedLast);
//    CGContextRef shadowContext = CGBitmapContextCreate(NULL, self.size.width + w, self.size.height + h, CGImageGetBitsPerComponent(self.CGImage), 0,
 ...
 *  @param interceptTime 截取时间
 *
 *  @return 截取的图片
 */
+ (UIImage *)thumbnailImageForVideo:(NSString *)videoURL atTime:(NSTimeInterval)interceptTime
{
    AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:[NSURL fileURLWithPath:videoURL] options:nil];
    AVAssetImageGenerator *gen = [[AVAssetImageGenerator alloc] initWithAsset:asset];
    gen.appliesPreferredTrackTransform = YES;
    CFTimeInterval thumbnailImageTime = interceptTime;
    CMTime time = CMTimeMakeWithSeconds(thumbnailImageTime, 600);  //  参数( 截取的秒数， 视频每秒多少帧)
    NSError *error = nil;
    CMTime actualTime;
