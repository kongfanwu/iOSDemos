...
@interface GKGLDiscountCouponVC ()<SPPageMenuDelegate, UIScrollViewDelegate>
...
@implementation GKGLDiscountCouponVC
...
- (void)initSubViews
{
    ...
    for (int i = 0; i < _dataArr.count; i++) {
        GKGLDiscountCouponSubVC * vc = [[GKGLDiscountCouponSubVC alloc] init];
        vc.userid = _userid;
        [self addChildViewController:vc];
        vc.tag = i;
        vc.GKGLDiscountCouponSubVCBlock = ^(NSString * _Nonnull count) {
            if (vc.tag == 0) {
                [weakSelf.pageMenu setTitle:[NSString stringWithFormat:@"可使用优惠券（%@）",count] forItemAtIndex:0];
            }
            if (vc.tag == 1) {
                [weakSelf.pageMenu setTitle:[NSString stringWithFormat:@"不可使用优惠券（%@）",count] forItemAtIndex:1];
            }
        };
        [self.myChildViewControllers addObject:vc];
    }
    if (self.pageMenu.selectedItemIndex < self.myChildViewControllers.count) {
        GKGLDiscountCouponSubVC *baseVc = self.myChildViewControllers[self.pageMenu.selectedItemIndex];
...
- (void)pageMenu:(SPPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex {
    ...
    GKGLDiscountCouponSubVC *targetViewController = self.myChildViewControllers[toIndex];
