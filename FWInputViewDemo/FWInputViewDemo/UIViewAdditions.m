...
+ (UIImage *)screenshot:(UIView *)view imageSize:(CGSize)imageSize
{
    UIGraphicsBeginImageContext(imageSize); //currentView 当前的view
    [view.layer renderInContext:UIGraphicsGetCurrentContext()];
    UIImage *viewImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    //    viewImage就是获取的截图，如果要将图片存入相册，只需在后面调用
...
- (void)setOpaque
{
    self.layer.opaque = YES; // 不透明
    self.layer.masksToBounds = YES; // 在iOS8开始，中文也会混合绘制，去掉透明区域。详情http://www.jianshu.com/p/db6602413fa3
}
