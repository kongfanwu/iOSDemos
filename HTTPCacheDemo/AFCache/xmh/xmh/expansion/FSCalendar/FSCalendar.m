...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        self.calendarWeekdayView.frame = CGRectMake(0, 10, self.contentView.fs_width, weekdayHeight);// TODO  self.calendarHeaderView.fs_bottom 不显示月份头,直接修改top的高度
        ...
        if (!self.floatingMode) {
            switch (self.transitionCoordinator.representingScope) {
                case FSCalendarScopeMonth: {
                    ...
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight + 15, self.fs_width, contentHeight); // TODO  直接修改top:+15的高度
                    _collectionView.frame = CGRectMake(0, 0, _daysContainer.fs_width, contentHeight);
                    break;
                }
                case FSCalendarScopeWeek: {
                    CGFloat contentHeight = rowHeight + padding*2;
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight + 15, self.fs_width, contentHeight);
                    _collectionView.frame = CGRectMake(0, 0, _daysContainer.fs_width, contentHeight);
                    break;
                }
            }
