...
@implementation LXCalendarView{
    LolCalendarModelList * _calendarModelList;
}
...
-(void)dealData:(LolCalendarModelList *)calenderModel{

    _calendarModelList = calenderModel;
    [self responData];
...
-(void)leftSlide{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"dateChange" object:_currentMonthDate];
...
//    NSDate *nextMonthDate = [self.currentMonthDate  nextMonthDate];
    ...
    LXCalendarMonthModel *monthModel = [[LXCalendarMonthModel alloc]initWithDate:_currentMonthDate];
...
//     LXCalendarMonthModel *nextMonthModel = [[LXCalendarMonthModel alloc]initWithDate:nextMonthDate];
    ...
    for (int i = 0; i <42; i++) {
        ...
        if (i >= firstWeekday && i < (firstWeekday + totalDays)) {
            ...
            if (model.day <=_calendarModelList.list.count) {
                model.dayModel = _calendarModelList.list[model.day-1];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"LXCalenderCell" owner:nil options:nil]lastObject];
        
    }
    if (cell.model.isCurrentMonth) {
        if (cell.model.day <=_calendarModelList.list.count) {
            cell.model.dayModel = _calendarModelList.list[cell.model.day-1];
        }
    }
...
-(void)layoutSubviews{
    ...
    self.calendarHeader.frame = CGRectMake(0, 0, self.width, 50);
}
#pragma mark---懒加载
-(LXCalendarHearder *)calendarHeader{
    if (!_calendarHeader) {
        _calendarHeader =[LXCalendarHearder showView];
        _calendarHeader.frame = CGRectMake(0, 0, self.width, 50);
...
-(LXCalendarWeekView *)calendarWeekView{
    if (!_calendarWeekView) {
        _calendarWeekView =[[LXCalendarWeekView alloc]initWithFrame:CGRectMake(0, self.calendarHeader.bottom, self.width, 50)];
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(self.width/7, 50);
        _collectionView =[[UICollectionView alloc]initWithFrame:CGRectMake(0, self.calendarWeekView.bottom, self.width, 6 * 50) collectionViewLayout:flow];
