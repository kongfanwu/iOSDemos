//
//  XMHSaleOrderStatisticTableView.m
//  xmh
//
...
@interface XMHSaleOrderStatisticTableView() <UITableViewDelegate, UITableViewDataSource,UITextViewDelegate>
/** <##> */
...
@implementation XMHSaleOrderStatisticTableView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {// 结算统计
        ...
        cell.yingfuPrice = self.yingfuPrice;
        [cell refresCellModelArr:_saleData];
        return cell;
    }else if (indexPath.section == 1){// 奖赠
        XMHAwardStatisticCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        if (!cell) {
            cell = [[XMHAwardStatisticCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"XMHAwardStatisticCellID"];
        }
        WeakSelf
        cell.addAward = ^{
            if (weakSelf.addAwardClick) {
                weakSelf.addAwardClick();
            }
        };
        cell.delectAward = ^(NSMutableArray *awardArr) {
            weakSelf.awardData = awardArr;
            [weakSelf reloadData];
        };
