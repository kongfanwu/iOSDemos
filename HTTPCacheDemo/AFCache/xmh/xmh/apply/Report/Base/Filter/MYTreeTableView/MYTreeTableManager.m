...
@property (nonatomic, assign) BOOL isAllCheck;
...
/** 反选   */
- (void)invertcheckChildItemWithItem:(MYTreeItem *)item isChildItemCheck:(BOOL)isChildItemCheck{
    
    if (item.checkState == MYTreeItemDefault) {
        item.checkState = MYTreeItemChecked;
    }else if (item.checkState == MYTreeItemChecked){
         item.checkState = MYTreeItemDefault;
    }else{}
    for (MYTreeItem *tmpItem in item.childItems) {
        // 如果是多选，勾选了 item 可以作用于子 item
        if (isChildItemCheck) {
            [self invertcheckChildItemWithItem:tmpItem isChildItemCheck:isChildItemCheck];
        } else {
            [self invertcheckChildItemWithItem:tmpItem isChildItemCheck:isChildItemCheck];
        }
    }
...
- (void)invertiCheck
{
    for (MYTreeItem *item in _showItems) {
        // 防止重复遍历
        if (item.level == 0) {
            [self invertcheckChildItemWithItem:item isChildItemCheck:YES];
        }
    }
}
...
- (void)getAllCheckItem:(NSMutableArray <MYTreeItem *>*)tmpArray andItem:(MYTreeItem *)tmpItem {
    ...
    /** 如果父节点 没选择  不遍历子节点 20190706 by ywl */
