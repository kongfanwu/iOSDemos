...
@implementation LApproveCellType6
...
- (void)setModel:(LApproveDetailModel *)model
{
    ...
    _lb1.text = [NSString stringWithFormat:@"%@的完善资料审批",model.data.initiator];
    ...
    _lb5.text = [NSString stringWithFormat:@"%@",model.state];
    _lb5.textColor = [ColorTools colorCauseByText:model.state];
    _lb6.text = [NSString stringWithFormat:@"%@",model.create_time];
    if (model.isRead ==0){
        _lbRead.hidden = NO;
    }else{
        _lbRead.hidden = YES;
